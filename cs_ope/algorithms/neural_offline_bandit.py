...
class ApproxNeuraLCBV2(BanditAlgorithm):
    ...
    def sample_action(self, contexts):
        ...
        for i in range(num_chunks):
            ...
            selected_actions = jnp.argmax(lcb, axis=1)

            # Convert selected actions to one-hot encoding
            onehot_actions = jax.nn.one_hot(selected_actions, num_classes=self.hparams.num_actions)

            # Append the one-hot actions for this chunk
            acts.append(onehot_actions)

        # Concatenate one-hot actions from all chunks and return
        return jnp.vstack(acts)
